...
冻结语料、case 与期望证据，并在报告中记录 corpus/case hash、embedding 模型与维度、索引及代码版本；把 index integrity、intent、retrieval、Evidence Gate、answer 和 full-agent 分层评测，避免只看最终平均分。`commit-ai-resolver` 当前对 27,646 条公开 React Commit 完成语义与路径增强，保留 75-case 工程回归集（52 dev / 23 frozen test），并从真实 GitHub Issue → closing PR → corpus fix commit 构建 461 条 RCA gold cases（当前为模型预审版）。461 条已经完整运行 Lexical/Dense/Hybrid Eval，Recall@10 分别为 0.385/0.694/0.658，并输出质量分、query 长度、领域、融合贡献与逐 case badcase；人工复核完成后再切分并冻结为 release-gate gold。完整 Agent runner 支持并发、重复采样、断点续跑和 trace，答案层可程序化检查引用有效性、证据覆盖、幻觉引用以及 Brier/ECE。当前尚未完成真实 LLM 全量答案评测，不能把 retrieval 全量 Eval 说成端到端 Agent 准确率。
...
**3 分钟展开版：**这个项目解决的是“哪次历史 Commit 可能引入或修复某个问题”。它与 Codex 在当前 checkout 上执行 `rg + git` 的一次性代码调查不同：目标数据跨仓库、跨日期、会被多人重复查询，所以先建立可复用的历史变更索引更经济、更稳定。离线侧以 daily JSON 为可审计 source of truth，对 27,646 条公开 React Commit 补齐完整 message、changed files 与 affected areas；语义文本进入 SQLite FTS5 与 sqlite-vec，repo、author、date、risk 等保留为 metadata，并用模型、维度、query instruction、文档模板和索引版本契约保证派生索引可重建。
...
评测侧有两层：75-case、52 dev / 23 frozen test 的版本化 Harness 负责工程回归；461 条真实 Issue → closing PR → fix commit 的 RCA gold cases（模型预审版）负责覆盖真实问题措辞和 RCA 语义差距，并已完成全量检索 Eval。23-case frozen regression test 的 Hybrid Recall@10 与 Evidence Gate 行为准确率为 100%；461-case 的 Lexical/Dense/Hybrid Recall@10 为 0.385/0.694/0.658，还暴露出等权 RRF 挤掉 Dense 命中的问题。我会明确前者不代表真实 RCA 100%，后者在完成人工复核前也不用于 release gate。下一步是在人工 gold dev split 上优化 query condensation、field-aware embedding 与融合，再增加 `git show/diff`、`rg` 和 symbol search，形成“索引粗召回 + Agentic 精查”的组合。
...
`commit-ai-resolver` 的检索与 Evidence Gate 回归基线已经建立，461 条 RCA gold cases 也已完成模型预审和全量 retrieval Eval，所以第一优先从“有没有数据集”转为完成四项人工 rubric、划分 gold dev/frozen test，并接入可用 chat API 跑完整 Agent 多次采样，验证引用忠实度、校准度、重试收益、P95 与成本；第二是按已发现的 fusion/长 query badcase 优化 query condensation、field-aware embedding 与 RRF；第三是把 trace、线上反馈和 ANN/并发/权限测试形成持续闭环。`adocag-server` 仍应先补检索 golden set 和回答 rubric，再增加动态规划。
...
先选择许可清晰、可复现的公开仓库并冻结 corpus hash；工程回归集按能力矩阵覆盖精确 SHA、语义查询、author/date、risk/date、repo/date、自然 OOD、虚构 ID 和需澄清请求。真实 RCA gold cases 则从 GitHub `Issue.closedByPullRequestsReferences` 出发，只保留 closing PR 已合并且 merge commit 存在于冻结 corpus 的链路，并保存 Issue/PR/Commit URL、关系和 hash。当前 461 条已完成模型预审与全量 retrieval Eval；下一步逐条人工确认问题忠实度、fix 关系、gold commit 完整性和 query 可用性，再冻结 gold dev/test。这样既能把 461 条称为正在建设并已全量评测的 gold cases，也不会把尚未人工复核的标签伪装成 release-ready gold。
...
它已经捕获多类会改变真实行为的问题：一是数据库只保存短 SHA 时，full SHA lookup 不能正确命中，随后补成完整值精确匹配与 7/8 位前缀兼容；二是 orchestrator 注入的默认 30 天窗口被误当成用户显式过滤条件，导致 Evidence Gate 对无关查询错误放行，随后改为只传递 intent 中显式提取的约束；三是 461-case RCA gold set 的全量 Eval 发现当前等权 RRF 只救回 9 条 Dense miss，却把 27 条 Dense top-10 hit 挤出 Hybrid top 10；四是 >1200 字符 issue query 的 Dense Recall@10 只有 0.653，低于中等长度组的 0.728。它还用结果 ID 集合检测 stale retry，并验证最大迭代参数不再被硬编码。能先暴露失败、再用固定 case 防复发，才是 Harness 的价值；用例数量或漂亮均分本身不是证明。
...
- **怎么做：**每轮保存 intent、候选、门禁、答案和 evaluation trace；跟踪 best-so-far；用 `repo:id` 集合比较相邻轮结果，若候选集合不变则判为 stale retry 并以 PARTIAL 结束；同时受 max iterations、token 与时间预算约束。当前 Synthesizer 普通查询最多读取 10 条 commit evidence、工单查询最多 15 条；Answer Evaluator LLM 只看答案和聚合检索元数据，不看逐条 commit，因此它负责质量分级和重试控制，引用真实性仍由独立确定性 scorer 校验。
...
- **怎么做：**冻结 corpus/case hash；75-case 中 52 dev 用于工程调参、23 frozen test 只做回归；另用 461 条真实 Issue/closing PR/fix commit RCA gold cases（模型预审版）完成全量检索基线和 badcase 分桶，待人工 rubric 通过后再切分 gold dev/frozen test。输出逐 case 排名、通道贡献、Recall@K、MRR、nDCG、拒答/澄清准确率、Brier/ECE、重试新颖度和 CI gate，并明确当前 461-case 指标不是 release gate 或生产准确率。
