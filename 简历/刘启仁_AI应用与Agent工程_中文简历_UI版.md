...
- 构建内部多仓库版本与外部公开复现版本：以 daily JSON 为可审计 source of truth，以 SQLite metadata/FTS5 与 sqlite-vec 为可重建派生索引；公开版本基于 27,000+ 条 React Commit 建库，并将 embedding 模型、1024 维度、query instruction、文档模板与索引版本纳入一致性契约。
...
- 建立版本化 Eval Harness，分层覆盖索引完整性、Intent、单通道召回、RRF、拒答/澄清、引用 grounding 与 Agent loop；输出 Recall@K、MRR、nDCG、Brier/ECE 与逐 case trace，并将 Hybrid Recall@10、Evidence Gate 行为准确率纳入 CI 回归门禁。
