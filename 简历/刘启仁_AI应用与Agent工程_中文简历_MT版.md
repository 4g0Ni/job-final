...
4 年微软 Microsoft Advertising 软件工程经验，工作主线覆盖 **AI Agent 与广告后端系统**。设计并实现面向多仓库历史变更与回归调查的 evidence-first 检索 Agent，具备混合召回、Agent 编排、MCP、拒答门禁、分层评测与可观测性的端到端实践；同时负责图片资产治理、Deal 同步、异步推荐视频及第三方测量等后端链路，并具备大型 Campaign UI 的端到端交付经验。
...
- 以 daily JSON 为可审计 source of truth、SQLite metadata/FTS5 与 sqlite-vec 为可重建派生索引，对 10,000 条公开 React Commit 建库；将 embedding 模型、1024 维度、query instruction、文档模板与索引版本纳入一致性契约。
...
- 建立 75-case 版本化 Eval Harness（52 dev / 23 frozen test），分层覆盖索引完整性、Intent、单通道召回、RRF、拒答/澄清、引用 grounding 与 Agent loop；输出 Recall@K、MRR、nDCG、Brier/ECE、逐 case trace 与 CI gate，当前 23-case frozen regression test 的 Hybrid Recall@10 与 Evidence Gate 行为准确率均为 100%。
